...
function display_vebinar_meta_box( $vebinar ) {
    ...
    <table>
        <tr>
            ...
            <td><input type="text" size="180" name="vebinar_price_name" value="<?php echo $vebinar_price; ?>" /></td>
        </tr>
        <tr>
            <td style="width: 100%">Дата проведения (Прим* 28 окт)</td>
            <td><input type="text" size="180" name="vebinar_data_name" value="<?php echo $vebinar_data; ?>" /></td>
        </tr>
        <tr>
            <td style="width: 100%">Время проведения (Прим* с 19 до 21)</td>
            <td><input type="text" size="180" name="vebinar_time_name" value="<?php echo $vebinar_time; ?>" /></td>
        </tr>
    </table>
    Прим* - Пример
    <?php
...
function display_author_meta_box( $vebinar ) {
    // Retrieve current name of the Director and Movie Rating based on review ID
    $author_name = esc_html( get_post_meta( $vebinar->ID, 'author_name', true ) );
    $author_position = esc_html( get_post_meta( $vebinar->ID, 'author_position', true ) );
    $author_about = esc_html( get_post_meta( $vebinar->ID, 'author_about', true ) );
    $author_photo = esc_html( get_post_meta( $vebinar->ID, 'author_photo', true ) );
    ?>
    <table>
    	<tr>
            <td style="width: 100%">Фото ведущего</td>
            <td><input type="file" size="180" name="vebinar_author_photo" value="<?php echo $author_photo; ?>" /></td>
        </tr>
        <tr>
            <td style="width: 100%">Имя ведущего</td>
            <td><input type="text" size="180" name="vebinar_author_name" value="<?php echo $author_name; ?>" /></td>
        </tr>
        <tr>
            <td style="width: 100%">Должность</td>
            <td><input type="text" size="180" name="vebinar_author_position" value="<?php echo $author_position; ?>" /></td>
        </tr>
        <tr>
            <td style="width: 100%">Информация про ведущего</td>
            <td><textarea type="text" cols="181" rows="5" name="vebinar_author_about"><?php echo $author_about; ?></textarea></td>
        </tr>
        <tr>
...
function add_vebinar_fields( $vebinar_id, $vebinar ) {
    ...
    if ( $vebinar->post_type == 'vebinar' ) {
        ...
        if ( isset( $_POST['vebinar_price_name'] ) && $_POST['vebinar_price_name'] != '' ) {
            update_post_meta( $vebinar_id, 'vebinar_price', $_POST['vebinar_price_name'] );
        }
        if ( isset( $_POST['vebinar_data_name'] ) && $_POST['vebinar_data_name'] != '' ) {
            update_post_meta( $vebinar_id, 'vebinar_data', $_POST['vebinar_data_name'] );
        }
        if ( isset( $_POST['vebinar_time_name'] ) && $_POST['vebinar_time_name'] != '' ) {
            update_post_meta( $vebinar_id, 'vebinar_time', $_POST['vebinar_time_name'] );
        }
        if ( isset( $_POST['vebinar_author_name'] ) && $_POST['vebinar_author_name'] != '' ) {
            update_post_meta( $vebinar_id, 'author_name', $_POST['vebinar_author_name'] );
        }
        if ( isset( $_POST['vebinar_author_position'] ) && $_POST['vebinar_author_position'] != '' ) {
            update_post_meta( $vebinar_id, 'author_position', $_POST['vebinar_author_position'] );
        }
        if ( isset( $_POST['vebinar_author_about'] ) && $_POST['vebinar_author_about'] != '' ) {
            update_post_meta( $vebinar_id, 'author_about', $_POST['vebinar_author_about'] );
        }
        if ( isset( $_POST['vebinar_author_photo'] ) && $_POST['vebinar_author_photo'] != '' ) {
            update_post_meta( $vebinar_id, 'author_photo', $_POST['vebinar_author_photo'] );
        }
